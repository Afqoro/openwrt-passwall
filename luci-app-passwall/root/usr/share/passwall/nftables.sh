...
filter_haproxy() {
	...
	echolog "  - [$?]Add load-balanced nodes to nftset[$NFTSET_VPS] Direct connection completed"
}
...
add_firewall_rule() {
	echolog "Starting to load nftables firewall rules..."
	gen_nft_tables
	...
	if [ -f $RULES_PATH/chnroute.nft ] && [ -s $RULES_PATH/chnroute.nft ] && [ $(awk 'END{print NR}' $RULES_PATH/chnroute.nft) -ge 8 ]; then
		#echolog "Use cache to load chnroute..."
	...
	if [ -f $RULES_PATH/chnroute6.nft ] && [ -s $RULES_PATH/chnroute6.nft ] && [ $(awk 'END{print NR}' $RULES_PATH/chnroute6.nft) -ge 8 ]; then
		#echolog "Use cache to load chnroute6..."
	...
	[ "$USE_DIRECT_LIST_ALL" = "1" ] && {
		...
		[ "$USE_GEOVIEW" = "1" ] && {
			...
			if [ -n "$GEOIP_CODE" ]; then
				...
				echolog "  - [$?] Parse and add [direct connection list] GeoIP to NFTSET completed"
			fi
		}
	}
	...
	[ "$USE_PROXY_LIST_ALL" = "1" ] && {
		...
		[ "$USE_GEOVIEW" = "1" ] && {
			...
			if [ -n "$GEOIP_CODE" ]; then
				...
				echolog "  - [$?] Parse and add [proxy list] GeoIP to NFTSET completed"
			fi
		}
	}
	...
	[ "$USE_BLOCK_LIST_ALL" = "1" ] && {
		...
		[ "$USE_GEOVIEW" = "1" ] && {
			...
			if [ -n "$GEOIP_CODE" ]; then
				...
				echolog "  - [$?] Parse and add [block list] GeoIP to NFTSET completed"
			fi
		}
	}
	...
	[ "$USE_SHUNT_NODE" = "1" ] && {
		...
		if [ -n "$GEOIP_CODE" ]; then
			...
			echolog "  - [$?] Parse and add [distribution node] GeoIP to NFTSET completed"
		fi
	}
	...
	if [ -n "${WAN_IP}" ]; then
		...
		for wan_ip in $WAN_IP; do
			echolog "  - [$?]Add WAN IPv4 to nftset[$NFTSET_WAN]：${wan_ip}"
		done
	fi
	unset WAN_IP wan_ip
	...
	[ "$PROXY_IPV6" == "1" ] && {
		...
		[ -n "${WAN6_IP}" ] && {
			...
			for wan6_ip in $WAN6_IP; do
				echolog "  - [$?]Add WAN IPv6 to nftset[$NFTSET_WAN6]：${wan6_ip}"
			done
		}
...
del_firewall_rule() {
	...
	echolog "Removing nftables rules is complete."
}

flush_nftset() {
	echolog "Clear the NFTSet."
