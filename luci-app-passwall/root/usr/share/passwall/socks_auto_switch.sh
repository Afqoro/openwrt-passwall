...
test_auto_switch() {
	...
	[ -z "$now_node" ] && {
		if [ -n "$(get_cache_var "socks_${id}")" ]; then
			...
			#echolog "Socks switch detection: unknown error"
			return 1
		fi
	}
	...
	if [ "$status" = "2" ]; then
		echolog "Socks switching detection: Unable to connect to the network, please check whether the network is normal!"
		return 2
	fi
	...
	if [ "$status" = "0" ]; then
		#echolog "Socks switching detection:${id}【$(config_n_get $now_node type)：[$(config_n_get $now_node remarks)]】normal."
		return 0
	elif [ "$status" = "1" ]; then
		local new_node msg
