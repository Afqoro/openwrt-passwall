		...
		for sid in $(ls -F ${TMP_ACL_PATH} | grep '/$' | awk -F '/' '{print $1}' | grep -v 'default'); do
			...
			for i in $(cat $_acl_list); do
				...
				if [ -n "${dns_redirect}" ]; then
					...
					[ -z "$(get_cache_var "ACL_${sid}_tcp_default")" ] && echolog "     - ${msg}DNS has been forcefully redirected to a dedicated DNS server using a different node than the global configuration."
				fi
...
filter_haproxy() {
	...
	echolog "  - [$?]Add load balancing nodes to ipset[$IPSET_VPS] Direct connection completed"
}
...
filter_vpsip() {
	...
	echolog "  - [$?]Add all IPv4 nodes to ipset[$IPSET_VPS] Direct connection completed"
	...
	echolog "  - [$?]Add all IPv6 nodes to ipset[$IPSET_VPS6] Direct connection completed"
	#订阅方式为直连时
...
add_firewall_rule() {
	echolog "Starting to load iptables firewall rules..."
	...
	[ "$USE_DIRECT_LIST_ALL" = "1" ] && {
		...
		[ "$USE_GEOVIEW" = "1" ] && {
			...
			if [ -n "$GEOIP_CODE" ]; then
				...
				echolog "  - [$?] Parse and add [direct connection list] GeoIP to IPSET completed"
			fi
		}
	}
	...
	[ "$USE_PROXY_LIST_ALL" = "1" ] && {
		...
		[ "$USE_GEOVIEW" = "1" ] && {
			...
			if [ -n "$GEOIP_CODE" ]; then
				...
				echolog "  - [$?] Parse and add [proxy list] GeoIP to IPSET completed"
			fi
		}
	}
	...
	[ "$USE_BLOCK_LIST_ALL" = "1" ] && {
		...
		[ "$USE_GEOVIEW" = "1" ] && {
			...
			if [ -n "$GEOIP_CODE" ]; then
				...
				echolog "  - [$?] Parse and add [block list] GeoIP to IPSET completed"
			fi
		}
	}
	...
	[ "$USE_SHUNT_NODE" = "1" ] && {
		...
		if [ -n "$GEOIP_CODE" ]; then
			...
			echolog "  - [$?] Parse and add [distribution node] GeoIP to IPSET completed"
		fi
	}
	...
	[ -n "$lan_ifname" ] && {
		...
		#echolog "Local IPv4 network segment mutual access direct connection:${lan_ip}"
		#echolog "Direct connection between local IPv6 network segments:${lan_ip6}"
	...
	[ ! -z "${WAN_IP}" ] && {
		...
		for wan_ip in $WAN_IP; do
			...
			echolog "  - [$?]Add WAN IPv4 to ipset[$IPSET_WAN]：${wan_ip}"
		done
	...
	[ ! -z "${WAN6_IP}" ] && {
		...
		for wan6_ip in $WAN6_IP; do
			...
			echolog "  - [$?]Add WAN IPv6 to ipset[$IPSET_WAN6]：${wan6_ip}"
		done
		...
		if [ -n "$TCP_NODE" ]; then
			_proxy_tcp_access() {
				...
				if echo "${2}" | grep -q -v ':'; then
					...
					[ $? -eq 0 ] && {
						echolog "  - Upstream DNS server${2}Already in the list for direct access, does not force forwarding of TCP/${3}Port access"
						return 0
					}
					if [ -z "${is_tproxy}" ]; then
					...
					echolog "  - [$?]Change the upstream DNS server [${2}]:${3}To join the TCP forwarding chain of the router's own proxy, please ensure that your node supports IPv6 and enable IPv6 transparent proxy!"
				fi
			}
		...
		if [ -n "$UDP_NODE" -o "$TCP_UDP" = "1" ]; then
			_proxy_udp_access() {
				...
				if echo "${2}" | grep -q -v ':'; then
					ipset -q test $IPSET_LAN ${2}
					[ $? == 0 ] && {
						echolog "  - Upstream DNS server${2}Already in the list for direct access, does not force forwarding of UDP/${3}Port access"
						return 0
					}
					...
					echolog "  - [$?]Change the upstream DNS server [${2}]:${3}To join the UDP forwarding chain of the router's own proxy, please ensure that your node supports IPv6 and enable IPv6 transparent proxy!"
				fi
			}
...
del_firewall_rule() {
	...
	echolog "Deletion of iptables rules is complete."
}
